...
class ChessPiece(object):
    ...
    def create_piece(self, rules):
        ...
        Computing long chains of numbers then just becomes a lookup 
        operation.
        ...
        for i in range(self.keypad.shape[0]):
            for j in range(self.keypad.shape[1]):
                ...
                piece[key] = []
                for rule in rules:
                    for subrule in rule: # ie -i, j vs i, j vs i, -j ...
                        piece[key].extend(
...
def compute_phone_numbers(piece):
    ...
    valid_start_keys = VALID_PHONE_NUMBER["valid_start_keys"]
    valid_length = VALID_PHONE_NUMBER["length"]
    for start_key in piece:
        
        if start_key not in valid_start_keys:
            continue

        new_phone_numbers = _move_piece(
            piece, start_key, 1, valid_length - 1)
...
def main():
    ...
    for piece_type in PIECES:
        ...
        valid_numbers = compute_phone_numbers(piece)
